...
import { list as listLoad, remove as removeItem } from '../../actions/roles';
...
class Roles extends React.Component {
    ...
    render = () => (
        <div>
            <Table data={this.props.list} query={this.props.query}
                ...
                <Column name="permissions" title="Права" template={
                    row => row.permissions.permissions.map((permission, i) =>
                        <span key={i}><span className={classnames('label', {
                            "label-success": permission.grantType == 10,
                            "label-danger": permission.grantType == 20,
                        })}>{permission.name}</span>&nbsp;{i != 0 && i % 4 == 0 && <br/>}</span>)
                } />
                <Column actions={true} template={
                    row =>
                        <ButtonGroup bsSize="xs">
                            <Button onClick={e => this.context.router.push(`/roles/${row.id}`)}><Glyphicon glyph="edit"/> Изменить</Button>
                            <Restrict permissions={permissions.RoleManage}>
                                <Button onClick={e => {
    ...
    renderActions = () =>
        <ButtonGroup bsSize="sm">
            <Restrict permissions={permissions.RoleManage}>
                <Button onClick={e => this.context.router.push('/roles/0')}><Glyphicon glyph="plus"/> Создать</Button>
            </Restrict>
        </ButtonGroup>
}
