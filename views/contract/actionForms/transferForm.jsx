...
class TransferForm extends React.Component {
    ...
    componentDidMount() {
        ...
        if (!readOnly) {
            ...
            this.props.change('reason', `Передача залогового билета №${contract.contractNumber} от ${moment(contract.contractDate).format('L')}`);
            this._init();
        }
    }
    ...
    _init(date) {
        ...
        let { supplyDebtSettings, supplyLoanSettings, supplyPenaltySettings } = rc.getSettings(options, contract.collateralType, new Date());
        ...
        if (contract && date) {
            ...
            // Долг
            let row10 = rc.buildRow10(rows, supplyDebtSettings, contract.loanCost);
            if (row10) {
            ...
            // Пошлина
            let row20 = rc.buildRow20(rows, contract, supplyLoanSettings, options, date);
            if (row20) {
            ...
            // Штраф
            let row30 = rc.buildRow30(rows, contract, supplyPenaltySettings, date);
            if (row30) {
    ...
    render() {
        ...
        return (
            <form onSubmit={this.props.handleSubmit}>
                ...
                <div className="row">
                    <FieldArray name="rows" component={ActionRows} readOnlyPercent={readOnlyDateAndPercents} onChangePercent={(e, val) => {
                        e.preventDefault();
...
export default TransferForm = reduxForm({
    form: 'TransferForm'
})(TransferForm);
