...
import ActionPositions from './actionPositions';

class PartialBuyoutForm extends React.Component {
    static contextTypes = {
    ...
    componentDidMount() {
        ...
        if (!readOnly) {
            ...
            this.props.change('reason', `Частичный выкуп залогового билета №${contract.contractNumber} от ${moment(contract.contractDate).format('L')}`);
            this._init();
        }
    }
    ...
    _init(date) {
        ...
        if (contract && date) {
            ...
            // Долг
            let row10 = rc.row10(rows, contract, options, this.calculateCost(positions));
            if (row10) {
    ...
    calculateCost(positions) {
        let cost = 0;
        if (positions && positions.length > 0) {
            for (let position of positions) {
                cost += parseInt(position.loanCost);
            }
        }
        return cost;
    }
    ...
    render() {
        ...
        return (
            <form onSubmit={this.props.handleSubmit}>
                ...
                <FieldArray name="data.positions" component={ActionPositions} readOnly={this.props.readOnly}
                            positions={this.props.contract.positions} onChange={() => this._init()} />
                <div className="row">
...
export default PartialBuyoutForm = reduxForm({
    form: 'PartialBuyoutForm'
})(PartialBuyoutForm);

const selector = formValueSelector('PartialBuyoutForm');